...
export default function ReferenceLibraryPage({
  ...
  return (
    <section className="data-page reference-library-page">
      <header>
        <div><span>本地预检与临时缓存</span><h1>音色参考</h1></div>
        <FileAudio2 size={24} />
      </header>
      <div className="reference-privacy-banner">
        <ShieldCheck size={20} />
        <div><strong>先在本机验证，生成前再次确认</strong><p>选择文件后只做格式、时长、大小与编码检查；尚未发送到云端。</p></div>
      </div>
      ...
      <div className="reference-library-list">
        {items.map((item, index) => (
          <article key={item.id}>
            ...
            <div><strong>{item.name}</strong><span>{item.duration_seconds.toFixed(1)} 秒 · {(item.bytes / 1024).toFixed(0)} KB · {item.codec}</span></div>
      ...
      {items.length === 0 ? <div className="empty-state compact"><FileAudio2 size={25} /><strong>还没有已检查的参考音频</strong><p>这里不会建立永久音色库。移除、成功生成或服务退出会立即清理；一小时以上缓存会在下次添加时清理。</p></div> : null}
    </section>
  );
}
